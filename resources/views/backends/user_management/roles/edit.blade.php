...
@section('title','Edit Staff Groups')
@section('page_heading','Edit Staff Groups')

@section('content')
    <div class="container-fluid">
        @include('backends.includes.notification')

        <form action="{{route('staff-groups.update',$role->id)}}" method="POST">
            @method('put')
            @csrf
            <div class="card">
                <div class="card-header" style="padding: 12px 20px;">
                    <h4 class="card-title">Edit Staff Group</h4>
                </div>
                <div class="card-body">
                    <div class="basic-form">
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="text" class="form-control form-control-sm @if($errors->has('name')) error-border-for-validation @endif" placeholder="Enter Group Name" name="name" value="{{$role->name}}">
                                @if($errors->has('name'))
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select class="form-control form-control-sm @if($errors->has('flag')) error-border-for-validation @endif" name="flag">
                                    @foreach($flagList as $flag)
                                        @if($flag == $role->flag)
                                            <option value="{{strtolower($flag)}}" selected>{{$flag}}</option>
                                        @else
                                            <option value="{{strtolower($flag)}}">{{$flag}}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @if($errors->has('flag'))
            ...
            @foreach($modules as $k => $module)
                <div class="card">
                    <div class="card-header" style="padding: 12px 20px;">
                        <label class="card-title"><input type="checkbox" @checked(in_array($module->id,$moduleIds)) name="select_module_id[]" onchange="selectModule(this,value)" style="height: 15px;width: 20px;" value="{{$module->id}}"/> {{$module->title}} Module</label>
                    </div>
