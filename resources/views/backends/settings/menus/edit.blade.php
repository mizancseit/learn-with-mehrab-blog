...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{route('menus.update',$id)}}" method="POST">
            @method('put')
            @csrf
                    ...
                    <div class="basic-form">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select class="form-control form-control-sm @if($errors->has('is_main_menu')) error-border-for-validation @endif" name="is_main_menu">
                                    @foreach($isMainMenuList as $is_main_menu_data)
                                        @if($is_main_menu_data['id'] == $data->is_main_menu)
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select class="form-control form-control-sm @if($errors->has('main_menu_id')) error-border-for-validation @endif" name="main_menu_id">
                                    ...
                                    @foreach($menus as $menu)
                                        <option value="{{$menu->id}}" @if($menu->id == $data->main_menu_id) @endif>{{$menu->title}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="number" class="form-control form-control-sm @if($errors->has('sort_by')) error-border-for-validation @endif" placeholder="Enter Sorting Value" name="sort_by" value="{{$data->sort_by ? $data->sort_by :0 }}" >
